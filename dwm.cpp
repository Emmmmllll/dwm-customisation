...
static void (*handler[LASTEvent]) (XEvent *) ={ //list of event handlers
	nullptr, // 0
	nullptr, // 1
	keypress, //[KeyPress] 2
...
void drawwinbar(Monitor *m) {
	if(winbarwin && m == selmon && m->sel){
...
updatebars(void)
{
	...
	// if(!winbarwin){
	// 	winbarwin = 
	// 	// XCreateWindow(dpy, root, m->wx, m->by, m->ww, bh, 0, DefaultDepth(dpy, screen),
	// 	// 		CopyFromParent, DefaultVisual(dpy, screen),
	// 	// 		CWOverrideRedirect|CWBackPixmap|CWEventMask, &wa);
	// 	XCreateSimpleWindow(dpy, root, 0, 0, 1, 1, 0, 0, 0);
	// 	// XCreateWindow(dpy, root, (m->wx + (m->ww / 3)), m->by, (m->ww / 3),
